...
static bool convert_arg(const char *arg, unsigned long long *val) {
	...
	// strtoll return an in-band error indicator; clear errno before the call
	errno = 0;
...
{
	size_t num_args;
	/* pass argv and argc, to convert_command_line_args(), update num_args and return NULL or pointer to allocated memory */
